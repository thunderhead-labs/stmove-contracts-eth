...
contract LockTest is Test {
    ...
    function testFuzz_Rebase(uint256 a, uint256 f, uint256 t0, uint256 t1, uint256 t2) public {
        ...
        vm.warp(t0);
        fstmove.rebaseByShareRate(10 ** 8, t0);
        fstmove.rebaseByShareRate(f, t2);
        ...
        if (t1 >= t2) {
            ...
            assertEq(fstmove.balanceOf(address(this)), a * f / 10 ** 8, "wrong fstmove balance; a");
        } else {
            uint256 rate = (f - 10 ** 8) * (block.timestamp - t0) / (t2 - t0) + 10 ** 8;

            assertEq(fstmove.shareRate(), rate, "wrong share rate; b");
            assertEq(fstmove.balanceOf(address(this)), a * rate / 10 ** 8, "wrong fstmove balance; b");
        }
